...
class DoubleLinkedList {
  ...
  get(i) {
    const len = this.length;
    if (i < 0 || i > len - 1) return null;
    if (i === len - 1) return this.tail;
    let ith;
  ...
  insert(i, val) {
    ...
    if (i === 0) {
      ...
      const prevToNewNode = this.get(i - 1);
      const nextToNewNode = prevToNewNode.next;
  ...
  remove(i) {
    ...
    if (i === 0) {
      ...
      const afterRemovable = removable.next;
      beforeRemovable.next = afterRemovable;
      afterRemovable.prev = beforeRemovable;
      this.length--;
      return removable;
    }
  }
...
const test = new DoubleLinkedList('first', 'second', 'third', 'forth');
